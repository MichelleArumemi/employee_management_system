...
export const NotificationProvider = ({ children }) => {
  ...
  // Send notification (call this from leaveContext or admin actions)
  const sendNotification = async (userId, message) => {
    try {
      await axios.post('/api/notifications', { userId, message });
      // Optionally refresh notifications for the user
    } catch (err) {
  ...
  return (
    <NotificationContext.Provider
      value={{
        ...
        markAsRead,
        deleteNotification,
        sendNotification, // <-- Add this to the context value
      }}
    >
      {children}
    </NotificationContext.Provider>
